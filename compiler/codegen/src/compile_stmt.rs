...
fn compile_method_call(
    ...
    // `THIS^`/`SUPER^` receivers are rejected earlier, during semantic
    // analysis (see `rule_method_call_declared.rs`, tracked in #1406), so
    // by codegen the receiver is always a plain instance variable.
    let instance = match &call.receiver {
        ironplc_dsl::textual::MethodReceiver::Instance(id) => id,
        ironplc_dsl::textual::MethodReceiver::SelfRef(_) => {
